...
import { clampWidgetPosition, FloatingWidget, formatCompactCount } from '../../src/userscript/floating-widget.ts';
...
function setup(saved?: string) {
  ...
  const storage = { value: saved ?? null, getItem: () => storage.value, setItem: (_key: string, value: string) => { storage.value = value; } };
  ...
  let cleared = false;
  const widget = new FloatingWidget({ document: document as never, window: window as never, storage, timer });
  return { widget, documentEvents, windowEvents, elementEvents, storage, window, get removed() { return removed; }, get cleared() { return cleared; } };
}
...
test('renders the autolike module shell and visible controls', () => {
  ...
  assert.match(app.widget.element.innerHTML, /value="combo"/);
